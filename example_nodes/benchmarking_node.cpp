...
//   args.query_point_current_body_frame = Vector3(0.0, 4.0, 0.0);
//   //int ni = 0;
...
// // void PoseCallback(nav_msgs::Odometry const& odom) {
...
// //   Vector3 pos = Vector3(odom.pose.pose.position.x, odom.pose.pose.position.y, odom.pose.pose.position.z);
...
void PointCloudCallback(const sensor_msgs::PointCloud2& msg) {
  ...
  args.axis_aligned_linear_covariance = Vector3(0.1, 0.1, 0.1);
  args.early_exit = false;
  args.query_point_current_body_frame = Vector3(curr_pose.position.x + 1.0, curr_pose.position.y + 1.0, curr_pose.position.z + 1.0);
  //int ni = 0;
  // for (float x = -rad; x <= rad; x =x+delta) {
  //   for (float y = -rad; y <= rad; y =y+delta) {
  //     for (float z = -rad; z <= rad; z =z+delta) {
